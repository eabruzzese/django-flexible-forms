...
def test_form_admin(django_assert_num_queries: Any, mocker: Any) -> None:
    ...
    with django_assert_num_queries(1):
        ...
        # The form should have one field.
        assert forms_admin._fields_count(form) == 1

        # The form should have zero records.
        assert ">0<" in forms_admin._records_count(form)

        # The form should link to the add_view with its app_form attribute set.
        assert f"?app_form={form.pk}" in forms_admin._add_record(form)
...
def test_record_admin(django_assert_num_queries: Any) -> None:
    ...
    # Concrete form fields are fields defined on the model class.
    concrete_form_fields = frozenset(
        f.name
        for f in AppRecord._meta.get_fields()
        if f.concrete and f.name not in ("id",)
    )

    # Flexible form fields are database-defined fields.
    flexible_form_fields = {f.name for f in test_form.fields.all()}
    ...
    # Calling get_form() with no instance or query parameters (i.e., adding a
    # new record from the admin list page) should return a form with only the
    # base fields since the record doesn't know what form it should be using
    # yet.
    record_form = records_admin.get_form(request=request, obj=None)
    assert frozenset(record_form().fields.keys()) == concrete_form_fields

    # Calling get_form() with a query parameter specifying the form to use
    # should return a form with fields.
    request.GET["app_form"] = test_form.pk
    record_form = records_admin.get_form(request=request, obj=None)
    assert frozenset(record_form().fields.keys()) == frozenset.union(
        concrete_form_fields, flexible_form_fields
    )
    ...
    # add_request = HttpRequest()
    # add_request.user = super_user
    # add_request.GET["app_form"] = test_form.pk
    # add_response = records_admin.add_view(add_request)
    ...
    # admin_form = records_admin.get_form(request=request, obj=added_record)
    # admin_form_fields = admin_form(instance=added_record).fields
    ...
    # # If the form has defined fieldsets, they should be rendered instead of the
    # # defaults. There should be two fieldsets: one defined by the user, and one
    # # containing top-level attributes of the record (in case it was
    # # customized).
